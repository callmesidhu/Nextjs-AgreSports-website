...
export default function HeroSection() {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      setTimeout(() => {
        setCurrentSlide((prev) => {
          ...
          if (nextSlide === 0) setScrollEnabled(true);
          return nextSlide;
        });
        setTextVisible(true);
      }, 300);
  ...
  return (
    ...
    >
      ...
      <div
        ref={glowRef}
        className={`fixed w-24 h-24 rounded-full bg-purple-700 filter blur-3xl pointer-events-none z-30 transform -translate-x-1/2 -translate-y-1/2 transition-opacity duration-700 ease-in-out ${isMoving ? 'opacity-120' : 'opacity-0'}`}
        style={{
          transition: 'opacity 0.0s ease-in-out, left 0.0s ease-out, top 0.0s ease-out',
        }}
      />
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center z-20 px-4">
        <div className="text-center">
          ...
          <div className="flex justify-center">
            <button
              className="px-8 py-3 bg-[#610bc6] text-white rounded-full transition-all duration-500 transform hover:bg-purple-600 hover:shadow-[0_0_20px_#610bc6] font-bold"
            >
              Connect Now
            </button>
          </div>
